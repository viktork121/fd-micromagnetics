...
class DemagField(object):
    ...
    def h(self, t, m):
        ...
        self._m_pad[:self._mesh.n[0],:self._mesh.n[1],:self._mesh.n[2],:] = m*self._Ms
        ...
        self._h = np.fft.irfftn(self._h_fft, axes = list(filter(lambda i: self._mesh.n[i] > 1, range(3))))
        #self._h_fft = np.fft.irfft(self._h_fft)
        return self._h[:self._mesh.n[0],:self._mesh.n[1],:self._mesh.n[2],:]
        
    def E(self, t, m):
        return - 0.5* constants.mu_0 * self._Ms * self._mesh.cell_volume * np.sum(m * self.h(t, m))
